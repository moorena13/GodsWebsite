...
<body>
	...
	<div class="container">
		...
		<div>
			<h1>Gods of
				<?php     
					$elementname = $pdo->query("SELECT * FROM Elements WHERE ElementID =     10"); 
			...
			<ul class="linklist">
				     <?php
					$results = $pdo->query("SELECT GreekGods.Name, GreekGods.GodID     FROM GreekGods JOIN GreekElement ON GreekGods.GodID = GreekElement.GreekID JOIN     Elements ON GreekElement.ElementID = Elements.ElementID WHERE Elements.ElementID = 10;");     
					foreach($results as $r)
					{
						     make_link($r['Name'], "g{$r['GodID']}");
					}
					     $results1 = $pdo->query("SELECT RomanGods.Name, RomanGods.GodID FROM     RomanGods JOIN RomanElement ON RomanGods.GodID = RomanElement.RomanID     JOIN Elements ON RomanElement.ElementID = Elements.ElementID WHERE     Elements.ElementID = 10;");
